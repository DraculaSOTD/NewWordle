...
const Game = () => {
    ...
    const [rows, setRows] = useState(
        new Array(Num_of_tries).fill(new Array(letters.length).fill(""))
    );
                ...
                {styles.map}>
                    {
                        rows.map((row, i) => 
                        (
                            <Animated.View existing={SlideInLeft.delay(i*30)} key={'row-color-${i}'} style={styles.row}>
                            {row.map(
                                (letter, j) => 
                                (
                                    <>
                                        {
                                            i < curRow &&
                                            (
                                                <Animated.View entering={FlipInEasyY.delay(j*100)} key={'cell-${i}-${j}'} style={getCellStyle(i, j)}>
                                        ...
                                        }
                                        {
                                            ...
                                            (
                                                <Animated.View entering={ZoomIn} key={'cell-active-${i}-${j}'} style={getCellStyle(i, j)}>
                                                    <Text style={styles.cellText}>{letter.toUpperCase()}
                                                    </Text>
                                                </Animated.View>
                                            )
                                        }
